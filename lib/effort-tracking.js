...
const questionsDirectory = path.join(process.cwd(), 'effort-tracking')
...
export function getSortedQuestionsData() {
  ...
  // Sort posts by date
  return allQuestionsData.sort(({ date: a }, { date: b }) => {
    if (a < b) {
      return 1
    } else if (a > b) {
      return -1
...
export function getAllEffortIds() {
  const fileNames = fs.readdirSync(questionsDirectory)
...
export async function getEffortData(id) {
